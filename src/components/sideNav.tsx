...
export default function SideNav({ open, onClose }: { open: boolean, onClose: () => void }) {
    return (
        <div
        className={`fixed left-0 md:top-19 top-16 md:w-60 sm:w-48 w-37 bg-[#ff1111]/80 shadow-lg transition-transform z-50
        h-[calc(100vh-64px)] ${open ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <button className="sm:ml-4 ml-2 text-white text-[18px] w-0 h-0 opacity-90 rounded-full bg-[#ff0000]/80 hover:scale-108 shadow-[0_10px_20px_rgba(0,0,0,0.3)] transition-all duration-300 cursor-pointer" onClick={onClose}>✕</button>
